...
#
#     # Imgur 이미지 URL 적용
#     image_url = "https://i.imgur.com/GCtegFI.png"  # 직접 이미지 URL을 사용
...
#             return (1 - humid / 100) * 0.6108 * (17.27 * temp) / (temp + 237.3)
#
...
#         vpd_checked = st.sidebar.checkbox("VPD")
#
#         # GDD 및 DLI는 하루 평균 집계 단위일 때만 선택 가능
#         if avg_option == "하루 평균":
#             gdd_checked = st.sidebar.checkbox("GDD (누적)")
#             dli_checked = st.sidebar.checkbox("DLI")
#         else:
#             gdd_checked = False
#             dli_checked = False
#
...
#             fig.add_trace(go.Scatter(x=filtered_data.index, y=filtered_data['VPD'], mode='lines', name="VPD"))
#         if gdd_checked and 'GDD' in data.columns:
#             fig.add_trace(go.Scatter(x=filtered_data.index, y=filtered_data['GDD'], mode='lines', name="GDD (누적)"))
#         if dli_checked and 'DLI' in data.columns:
#             fig.add_trace(go.Scatter(x=filtered_data.index, y=filtered_data['DLI'], mode='lines', name="DLI"))
#
#         # 그래프 레이아웃에 x축과 y축의 단위 추가
#         fig.update_layout(
#             title="환경 데이터 시각화",
#             xaxis_title="시간",
#             yaxis_title="값",
...
#         if radn_checked and 'radn' in data.columns:
#             selected_traces += 1
#             offset = 50 * (selected_traces - 2)  # 오프셋 추가
#             fig.add_trace(go.Scatter(x=filtered_data.index, y=filtered_data['radn'] + offset, mode='lines', name="일사(W/㎡)"))
#         if wind_checked and 'wind' in data.columns:
#             selected_traces += 1
#             offset = 50 * (selected_traces - 2)
#             fig.add_trace(go.Scatter(x=filtered_data.index, y=filtered_data['wind'] + offset, mode='lines', name="1분평균풍속(m/s)"))
#         if rainfall_checked and 'rainfall' in data.columns:
#             selected_traces += 1
#             offset = 50 * (selected_traces - 2)
#             fig.add_trace(go.Scatter(x=filtered_data.index, y=filtered_data['rainfall'] + offset, mode='lines', name="강우(mm)"))
#         if battery_checked and 'battery' in data.columns:
#             selected_traces += 1
#             offset = 50 * (selected_traces - 2)
#             fig.add_trace(go.Scatter(x=filtered_data.index, y=filtered_data['battery'] + offset, mode='lines', name="배터리 전압(V)"))
#         if vpd_checked and 'VPD' in data.columns:
#             selected_traces += 1
#             offset = 50 * (selected_traces - 2)
#             fig.add_trace(go.Scatter(x=filtered_data.index, y=filtered_data['VPD'] + offset, mode='lines', name="VPD"))
#         if gdd_checked and 'GDD' in data.columns:
#             selected_traces += 1
#             offset = 50 * (selected_traces - 2)
#             fig.add_trace(go.Scatter(x=filtered_data.index, y=filtered_data['GDD'] + offset, mode='lines', name="GDD (누적)"))
#         if dli_checked and 'DLI' in data.columns:
#             selected_traces += 1
#             offset = 50 * (selected_traces - 2)
#             fig.add_trace(go.Scatter(x=filtered_data.index, y=filtered_data['DLI'] + offset, mode='lines', name="DLI"))
#
#         # 그래프 레이아웃에 x축과 y축의 단위 추가
#         fig.update_layout(
#             title="환경 데이터 시각화",
#             xaxis_title="시간",
#             yaxis=dict(title=first_axis_name, titlefont=dict(color="blue")),
#             yaxis2=dict(title=second_axis_name, overlaying="y", side="right", titlefont=dict(color="red")),
#             legend_title="데이터 종류",
#             hovermode="x",
#             showlegend=True
#         )
...
if menu == "📘 사용법 안내":
    ...
    if "data" not in st.session_state:
        ...
        if vpd_checked and 'VPD' in data.columns:
            selected_traces += 1
            offset = 50 * (selected_traces - 2)
            fig.add_trace(go.Scatter(x=filtered_data.index, y=filtered_data['VPD'] + offset, mode='lines', name="VPD (kPa)"))
        if gdd_checked and 'GDD' in data.columns:
            selected_traces += 1
            offset = 50 * (selected_traces - 2)
            fig.add_trace(go.Scatter(x=filtered_data.index, y=filtered_data['GDD'] + offset, mode='lines', name="GDD (°C)"))
        if dli_checked and 'DLI' in data.columns:
            selected_traces += 1
            offset = 50 * (selected_traces - 2)
            fig.add_trace(go.Scatter(x=filtered_data.index, y=filtered_data['DLI'] + offset, mode='lines', name="DLI (mol/m²/s)"))
